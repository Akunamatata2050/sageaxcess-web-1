(function() {
  ...
  function FirstTimeEntitiesCtrl($scope, $location, $route, FirstTimeEntitiesSvc, dialogs, $rootScope) {
    var vm = this;
    var original;
    ...
    $scope.$on('updatedUsersList', function(){
      FirstTimeEntitiesSvc.list().then(function(resp) {
        vm.users = resp;
      });
    });
    ...
    function getUsers() {
      ...
      if (!params.id) {
        ...
        FirstTimeEntitiesSvc.get(params.id).then(function(resp) {
          if(!resp.Active){
            gotoListPage();
          }
          vm.user = resp;
    ...
    function removeUser() {
      var dlg;
      dlg = dialogs.confirm('Remove "' + vm.user.FirstName+' '+vm.user.LastName + '" User?', '');
    ...
    function gotoListPage(){
      $location.path('/first-time-users');
    }
  }

  FirstTimeEntitiesCtrl.$inject = [
    '$scope', '$location', '$route', 'FirstTimeEntitiesSvc', 'dialogs', '$rootScope'
  ];

  angular.module('theme.core.main_controller').controller('FirstTimeEntitiesCtrl', FirstTimeEntitiesCtrl);
}).call(this);
