(function() {
  ...
  function FirstTimeUsersCtrl($scope, $location, $route, FirstTimeUsersSvc, dialogs, $rootScope) {
    var vm = this;
    var original;
    ...
    $scope.$on('updatedUsersList', function(){
      FirstTimeUsersSvc.list().then(function(resp) {
        vm.users = resp;
      });
    });
    ...
    function getUsers() {
      ...
      if (!params.id) {
        ...
        FirstTimeUsersSvc.get(params.id).then(function(resp) {
          if(!resp.Active){
            gotoListPage();
          }
          vm.user = resp;
  ...
  FirstTimeUsersCtrl.$inject = [
'$scope', '$location', '$route', 'FirstTimeUsersSvc', 'dialogs', '$rootScope'
];

angular.module('theme.core.main_controller').controller('FirstTimeUsersCtrl', FirstTimeUsersCtrl);
}).call(this);
